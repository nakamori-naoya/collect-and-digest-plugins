...
COLLECTION_KEYS = ("include_subagents", "max_scan_files", "max_source_bytes", "stable_read_retries", "quiescent_after_minutes")


def validate_config(cfg, path):
    """collect-sessions設定の閉じたschema検査。key集合・型・値の許容範囲を見る。fallbackは無い。"""
    if not isinstance(cfg, dict) or set(cfg) != {"version", "state_dir", "timezone", "sources", "collection"}:
        fail("設定のtop-level keyがschemaと一致しない（version / state_dir / timezone / sources / collection）: {}".format(path))
    if type(cfg["version"]) is not int or cfg["version"] != 1:
        fail("versionは1だけを受け付ける")
    for key in ("state_dir", "timezone"):
    ...
    if not isinstance(sources, dict) or set(sources) != set(SOURCES):
        fail("sources のkeyは claude_code / codex")
    for name in SOURCES:
        spec = sources[name]
        if (not isinstance(spec, dict) or set(spec) != {"enabled", "root"} or type(spec["enabled"]) is not bool
                or not isinstance(spec["root"], str) or not spec["root"]):
            fail("sources.{} は enabled(boolean) と root(文字列) だけを持つ".format(name))
    if not any(sources[name]["enabled"] for name in SOURCES):
        fail("sourceを1つ以上有効にすること")
    collection = cfg["collection"]
    if not isinstance(collection, dict) or set(collection) != set(COLLECTION_KEYS):
        fail("collection のkeyは include_subagents / max_scan_files / max_source_bytes / stable_read_retries / quiescent_after_minutes")
    if type(collection["include_subagents"]) is not bool:
        fail("collection.include_subagents はboolean")
    for key in COLLECTION_KEYS[1:]:
        if type(collection[key]) is not int or collection[key] <= 0:
            fail("collection.{} は正の整数".format(key))
    try:
...
def load_config(raw):
    """利用者設定 ${XDG_CONFIG_HOME:-~/.config}/harness-plugins/collect-sessions.config.yml を直接読み、
    schemaを検査し、state_dir と各 source root の ~ を展開して絶対pathにして返す。相対pathは受け付けない。"""
    raw = raw.strip()
    if raw.startswith("{"):
        cfg = json.loads(raw)
    else:
        ...
        if result.returncode:
            fail("設定fileを読めない: {}".format(raw))
        cfg = json.loads(result.stdout)
    validate_config(cfg, raw)
    cfg = json.loads(json.dumps(cfg))
    cfg["state_dir"] = os.path.expanduser(cfg["state_dir"])
    if not os.path.isabs(cfg["state_dir"]):
        fail("state_dir は絶対path（~ 展開可）で書くこと")
    for name in SOURCES:
        root = os.path.expanduser(cfg["sources"][name]["root"])
        if not os.path.isabs(root):
            fail("sources.{}.root は絶対path（~ 展開可）で書くこと".format(name))
        cfg["sources"][name]["root"] = root
    return cfg
    ...
    for source, path in source_files(cfg):
        try:
            ...
            items.append({
                ...
                "collector": "collect-sessions/" + plugin_version(),
