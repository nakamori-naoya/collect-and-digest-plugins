...
COLLECTOR = "collect-notes/" + _plugin_version()
...
def validate_config(cfg, path):
    """collect-notes設定の閉じたschema検査。key集合・型・値の許容範囲を見る。fallbackは無い。"""
    if not isinstance(cfg, dict) or set(cfg) != {"version", "notes_dir", "timezone", "collect"}:
        fail("設定のtop-level keyがschemaと一致しない（version / notes_dir / timezone / collect）: {}".format(path))
    if type(cfg["version"]) is not int or cfg["version"] != 1:
        fail("versionは1だけを受け付ける")
    for key in ("notes_dir", "timezone"):
        if not isinstance(cfg[key], str) or not cfg[key]:
            fail("{} は空でない文字列".format(key))
    collect = cfg["collect"]
    if not isinstance(collect, dict) or set(collect) != {"sources", "transcript", "max_bytes"}:
        fail("collect のkeyは sources / transcript / max_bytes")
    if type(collect["transcript"]) is not bool:
        fail("collect.transcript はboolean")
    if type(collect["max_bytes"]) is not int or collect["max_bytes"] <= 0:
        fail("collect.max_bytes は正の整数")
    sources = collect["sources"]
    if not isinstance(sources, dict) or set(sources) - {"notion", "google_docs"}:
        fail("collect.sources に未知のsourceがある")
    for name, spec in sources.items():
        allowed = {"enabled"} | ({"drive_query"} if name == "google_docs" else set())
        if not isinstance(spec, dict) or set(spec) - allowed or type(spec.get("enabled")) is not bool:
            fail("collect.sources.{} のkeyまたは型が不正".format(name))
        if "drive_query" in spec and not isinstance(spec["drive_query"], str):
            fail("collect.sources.google_docs.drive_query は文字列")
    try:
...
def load_config(raw):
    """<repo>/.harness-plugins/collect-notes.config.yml を直接読み、schemaを検査し、
    notes_dir をrepository root（設定fileの2つ上）基準の絶対pathへ解決して返す。"""
    raw = raw.strip()
    if raw.startswith("{"):
        cfg = json.loads(raw)
        base = os.getcwd()
        ...
        try:
            result = subprocess.run(["yq", "-o=json", "-I=0", ".", str(path)], capture_output=True,
                                    text=True, timeout=10, check=True)
            cfg = json.loads(result.stdout)
        except (OSError, subprocess.SubprocessError, json.JSONDecodeError) as exc:
            fail("設定fileを読めない: {} ({})".format(raw, exc))
        base = str(path.resolve().parent.parent)
    validate_config(cfg, raw)
    cfg = dict(cfg)
    notes = os.path.expanduser(cfg["notes_dir"])
    if not os.path.isabs(notes):
        notes = os.path.join(base, notes)
    cfg["notes_dir"] = os.path.abspath(notes)
    cfg["repo_root"] = base
    return cfg
