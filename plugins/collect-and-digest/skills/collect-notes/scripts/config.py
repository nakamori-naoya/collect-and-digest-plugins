#!/usr/bin/env python3
"""collect-notes の設定fileを検査して読む唯一の入口。呼び手はpathを選ばない。

  python3 scripts/config.py check --repo <repository_path>
  python3 scripts/config.py read --repo <repository_path>

設定fileは <repositoryのgit root>/.harness-plugins/collect-notes.config.yml に固定する（1層・fallback無し）。
...
schemaの正式な定義は同じdirectoryの note.py の validate_config で、このtoolはそれを共有する。
"""
import sys
...
import note  # noqa: E402  同じ入口のtool。validate_config だけを使う

if __name__ == "__main__":
    harness_config.main("collect-notes", "repository", note)
