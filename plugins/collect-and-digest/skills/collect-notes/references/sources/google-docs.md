...
Drive のクエリ構文で `mimeType` と時刻を指定する。日付は対象日の設定 `timezone` の 0:00をUTCへ直して渡す。翌日境界も翌日のローカル0:00から求め、24時間固定にしない。
...
Gemini の議事録では、要約と文字起こしが同一 Doc の別セクションにある場合と、別ファイルにある場合がある。どちらなのかは、収集のたびに取得した実物の構造を見て決める。
