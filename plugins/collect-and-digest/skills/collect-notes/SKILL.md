...
- MCPが使えないsourceがある。理由を記録してスキップし、有効な残りのsourceを続ける。
- 対象日の指定が無い、または `occurred_at` が取れない。設定の `timezone` の当日、または上流の作成日時で対象日を決め、報告にその根拠を書く。
- 本文・参加者・録画URLの一部が上流から取れない。取れた範囲を原文のまま保存し、省略項目を記録する。推定で補わない。
