...
- **依頼はどのdigest定義に当たるか。** 依頼を `digests[].name` と `period` へ照合し、`digest_name` と基準時刻（省略時は今日）を確定する。当たらなければ候補を示して一問で確認する。
...
- 設定fileが無い、schema に合わない、`type` が `period-digest` 以外。診断を報告して止まる。
- 依頼がどの `digests[].name` にも当たらない。候補を示して確認する。
- 素材が0件。書かずに報告して終わる。
