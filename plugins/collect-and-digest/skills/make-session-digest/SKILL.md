...
- **何を残し、何を混ぜないか。** material の `source_path` は要約時だけ読み、全文や中間要約を保存しない。material自体もfileに書かない。残すもの・混ぜないものは[privacy境界](references/privacy.md)、本文・metadata・タグの形は[日次記録の契約](references/output.md)に従う。各セッションから後で仕事へ再利用できる事実と判断を選び、必要な根拠を文字数で機械的に削らない。
