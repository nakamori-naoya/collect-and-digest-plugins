...
これは「意味で伏せない」に対するただ一つの例外である。「これは機密っぽい」という意味の判断で伏せることはしない。意味の判断には検出漏れが避けられず、伏せたから安全だという誤った安心を与えるからである。ここで行うのは固定フォーマットへの文字列一致だけで、判断も停止も挟まないので、意味で伏せる処理とは別物である。
...
保存先はgit管理下に置かない。設定で無効にした対象は取らない。
